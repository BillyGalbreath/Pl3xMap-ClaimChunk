...
import net.pl3x.map.claimchunk.task.Pl3xMapTask;
    ...
    public Pl3xMapHook(Plugin plugin) {
        Pl3xMapProvider.get().mapWorlds().forEach(world -> {
            SimpleLayerProvider provider = SimpleLayerProvider
                    ...
                    .build();
            world.layerRegistry().register(Key.of("claimchunk_" + world.uuid()), provider);
            Pl3xMapTask task = new Pl3xMapTask(world, provider);
            task.runTaskTimerAsynchronously(plugin, 20, 20L * Config.UPDATE_INTERVAL);
